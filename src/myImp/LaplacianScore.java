...
public class LaplacianScore {
	...
	public static int[] laplacianScore(double[][] matrix) {
		...
		double[][] sumMatrix = MatrixTools.vectorToMatrix(sum);
		double[][] sumMatrixT = MatrixTools.transpose(sumMatrix);

		double[][] tmpl = MatrixTools.multiplicateMatrixes(sumMatrixT, matrix);

		double[][] D = MatrixTools.transformToDiagonalMatrix(sum);

		double sumOfAllElements = MatrixTools.sumOfElements(sum);
		MatrixTools.transformValues(tmpl, x -> x * x / sumOfAllElements);

		double[][] DPrime = calculatePrime(matrix, D);
		DPrime = MatrixTools.useFunctionOnMatrixes((a, b) -> a - b, DPrime, tmpl);

		double[][] LPrime = calculatePrime(matrix, distanceMatrix);
	...
	private static double[][] calculatePrime(double[][] elementsMatrix, double[][] multiplicantMatrix) {
		double[][] elementsMatrixTrans = MatrixTools.transpose(elementsMatrix); // X'

		double[][] LPrime = MatrixTools.multiplicateMatrixes(elementsMatrixTrans, multiplicantMatrix); // X'*L

		LPrime = MatrixTools.transpose(LPrime); // (X'*L)'
		LPrime = MatrixTools.useFunctionOnMatrixes((a, b) -> a * b, LPrime, elementsMatrix); // (X'*L)'.*X

		double[] sumOfLPrime = MatrixTools.sumOfElements(LPrime, false); // sum((X'*D)'.*X)

		double[][] matrixOfSumOfLPrime = MatrixTools.vectorToMatrix(sumOfLPrime);
		matrixOfSumOfLPrime = MatrixTools.transpose(matrixOfSumOfLPrime);

		return matrixOfSumOfLPrime;
	}
}
